...
| 3 | 2 capability rischiose + prompt meta | 3 | `feat-007` + `feat-008` + `feat-meta` | Disponibili: `competence-gap`, `pt-analysis`. Inoltre `meta-prompt-genera-profilo.md` consente a Denis di esplorare nuove capability in autonomia. | **deployata 2026-05-21** (zip `dist/labnexus-sprint1-darwin-arm64.zip` 31 file, 8.6 MB — 7 profili + meta-prompt + guida + KB + .app + README). **L2 Denis F+G + FR-22 3 casi Claude esterno + L2 usabilità feat-meta** = gate di chiusura formale Sprint 1 (fuori pipeline AI). |
