...
| 1.5.B | Config master TOML + parser nuovi formati | 9-13h | FR-11 → FR-24 | `labnexus.config.toml` master con defaults globali. Profili `.toml` con campi opzionali (eredità da master, override locale). `trigger_prompt_file` indirezione (XOR con `trigger_prompt` inline, path-safe). Parser `.xls`, `.doc`, `.rtf` in `internal/input`. Meta-prompt aggiornato a TOML. Tabella ownership chiavi frontmatter documentata. | pianificata |
| 1.5.C | Concierge mode + auto-discovery + audit log + streaming live | 9-16h | FR-25 → FR-39 (+ EC-1 → EC-16) | Cartella `lavori/` rinominata da `data/`. Auto-discovery via `_labnexus.toml` per cartella + fallback convention naming. Subcommand `labnexus jobs` + flag `--job <name>`. TUI estesa con menu pre-mappato. Logger multi-writer (stderr + file `.log` per ogni run) come audit trail ISO 17025. Body streaming live in stderr + log mirror. Output dentro `lavori/<X>/output/`. Documenti aggiornati con tono concierge italiano. | pianificata |
