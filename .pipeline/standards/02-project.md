...
- **Modello default Sprint 1.5.C+**: stringa `qwen3.5-122b-a10b` (Qwen3.5 122B A10B Instruct su eurouter, thinking mode + tools + JSON, MoE 122B totali / 10B activated, 262K context). Default temperature `0.9`, `max_tokens 8192`, `context_window 262000`. Storico Sprint 1: era `qwen3.6` (Ollama-style tag) con `context_window 128000` — sostituito 2026-05-22 post-smoke perché eurouter non riconosceva `qwen3.6` (catalogo eurouter usa naming `qwen3.5-...` lowercase con dashes). Per Ollama locale (provider opzionale), il modello equivalente resta `qwen3.6` (tag Ollama).
